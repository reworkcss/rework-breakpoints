...
function apply() {
  ...
  for (var i = 0; i < maxPoints.length; i++) {
    ...
    if (i) {
      media[maxPoints[i].name] += ' and (min-device-width: ' + (maxPoints[i-1].value + maxPoints[i-1].factor) + maxPoints[i-1].unit + ')';
      media[maxPoints[i].name + '-and-down'] = 'only screen and (max-device-width: ' + maxPoints[i].value + maxPoints[i].unit + ')';
      media[maxPoints[i].name + '-and-up'] = 'only screen and (min-device-width: ' + (maxPoints[i-1].value + maxPoints[i-1].factor) + maxPoints[i-1].unit + ')';
    }
    media[maxPoints[i].name] += ' and (max-device-width: ' + maxPoints[i].value + maxPoints[i].unit + ')';
  }
  ...
  for (var j = minPoints.length - 1; j >= 0; j--) {
    ...
    media[minPoints[j].name] += ' and (min-device-width: ' + minPoints[j].value + minPoints[j].unit + ')';
    if (j < minPoints.length - 1) {
      media[minPoints[j].name] += ' and (max-device-width: ' + (minPoints[j+1].value - minPoints[j+1].factor) + minPoints[j+1].unit + ')';
      media[minPoints[j].name + '-and-up'] = 'only screen and (min-device-width: ' + minPoints[j].value + minPoints[j].unit + ')';
      media[minPoints[j].name + '-and-down'] = 'only screen and (max-device-width: ' + (minPoints[j+1].value - minPoints[j+1].factor) + minPoints[j+1].unit + ')';
    }
  }
