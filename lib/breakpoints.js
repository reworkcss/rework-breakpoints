...
function visit(declarations) {
  for (var i = 0; i < declarations.length; i++) {
    ...
    if (typeof decl.property !== 'string' || typeof decl.value !== 'string') {
      continue;
    }
...
function apply() {
  var media = {},
      ...
      screen = options['use-only'] ? 'only screen' : 'screen';
  ...
  for (var i = 0; i < maxPoints.length; i++) {
    media[maxPoints[i].name] = screen;
    if (i) {
      media[maxPoints[i].name] += ' and (min' + query + ': ' + getValue(maxPoints[i-1], 1) + ')';
      media[maxPoints[i].name + '-and-down'] = screen + ' and (max' + query + ': ' + getValue(maxPoints[i]) + ')';
      media[maxPoints[i].name + '-and-up'] = screen + ' and (min' + query + ': ' + getValue(maxPoints[i-1], 1) + ')';
    }
  ...
  for (var j = minPoints.length - 1; j >= 0; j--) {
    ...
    if (j < minPoints.length - 1) {
      ...
      media[minPoints[j].name + '-and-up'] = screen + ' and (min' + query + ': ' + getValue(minPoints[j]) + ')';
      media[minPoints[j].name + '-and-down'] = screen + ' and (max' + query + ': ' + getValue(minPoints[j+1], -1) + ')';
    }
  }
